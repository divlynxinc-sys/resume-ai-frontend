...
function modernMinimal(data) {
  ...
  const emailIcon = `<svg class="mm-ci" viewBox="0 0 24 24" fill="none" stroke="${META}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="22,6 12,13 2,6"/></svg>`;
  const phoneIcon = `<svg class="mm-ci" viewBox="0 0 24 24" fill="${META}" aria-hidden="true"><path d="M20 15.5c-1.25 0-2.45-.2-3.57-.57a1 1 0 0 0-1.02.24l-2.2 2.2a15.05 15.05 0 0 1-6.59-6.59l2.2-2.21a1 1 0 0 0 .24-1.02A11.36 11.36 0 0 1 8.5 4a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1c0 9.39 7.61 17 17 17a1 1 0 0 0 1-1v-3.5a1 1 0 0 0-1-1z"/></svg>`;
  ...
  const css = `
    ...
    .resume-root .contact-sep { display: inline-block; margin: 0 4px; color: #bbb; vertical-align: middle; }
    .resume-root .mm-ci-item { display: inline-flex; align-items: center; gap: 4px; white-space: nowrap; vertical-align: middle; }
    .resume-root .mm-ci {
      display: block;
      width: 11px;
      height: 11px;
    ...
    .resume-root .section-label { display: block; }
  ...
  `;
  const section = (title, content) => content.trim() ? `<div class="section"><div class="section-title"><div class="section-label">${title}</div><div class="section-rule"></div></div>${content}</div>` : "";
  const summaryHtml = r.summary ? `<p class="summary">${esc(r.summary)}</p>` : "";
  const bulletList = (bullets) => `<div class="bullets">${bullets.map((b) => `<div class="bullet-row">&bull; ${esc(b)}</div>`).join("")}</div>`;
  ...
  const projectsHtml = r.projects.map((proj) => {
    ...
    const linkHtml = link ? `<a class="proj-url" href="${esc(link.href)}">${esc(link.text)}</a>` : "";
...
function classicProfessional(data) {
  ...
  const header = `
    ...
    <div class="contact">${contactParts.join('<span class="ci-sep">\xB7</span>')}</div>
  `;
