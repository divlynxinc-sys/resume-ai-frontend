...
`src/features/ai-interviews/` — a real-time spoken mock interview over **LiveKit** (not the record-and-upload flow older docs describe). `page.tsx` = six screens by pathname; `live-room.tsx` = the LiveKit room (`livekit-client`, `@livekit/components-react`, no components-styles); `api.ts` = `LiveInterviewApi` mapping `services/interviews.ts` DTOs → UI types. Flow: `POST /interviews` → `/ready` mic test → `POST /interviews/{id}/start` returns `{url, token}` → `/live` joins room → the backend-dispatched worker (`../resumeai-AI/interview_agent`, agent "Sam") runs the interview → room closes → `/processing` polls `GET /interviews/{id}` → `/report`. **Credit-based, not plan-gated (2026-09-14):** 1 credit = 1 interview + its report, sold as one-time packs (3 for $10, 30 for $90) to any signed-in user. Display catalog `src/lib/interview-credits.ts` (static, must match backend `InterviewCreditSettings` + the Polar products); UI in `src/components/shared/interview-credits.tsx` (`useInterviewCredits`, pack cards, `InterviewCreditsAddon` on `/pricing`, `InterviewCreditsModal` mounted by the interviews page). A 402 `interview_credits_required` makes `lib/api.ts` dispatch `interview-credits-required` (not `upgrade-required`). `/success?purchase=interview_credits` polls `POST /interview-credits/sync`. **The worker must be running** (`uv run python agent.py dev` in `resumeai-AI/interview_agent`) or the candidate sits in an empty room.
